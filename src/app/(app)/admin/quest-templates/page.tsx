...
const DIFF_COLOR: Record<PendingTemplate["difficulty"], string> = {
  EASY: "text-neon-green bg-neon-green/10",
  MEDIUM: "text-blue-400 bg-blue-400/10",
  HARD: "text-neon-orange bg-neon-orange/10",
  LEGENDARY: "text-neon-red bg-neon-red/10",
};
...
export default function AdminQuestTemplatesPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetch(`/api/admin/quest-templates?status=${tab}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  async function approve(id: string) {
    setActingOn(id);
    try {
      const res = await fetch(`/api/admin/quest-templates/${id}`, {
        method: "PATCH",
  ...
  async function reject() {
    ...
    try {
      const res = await fetch(`/api/admin/quest-templates/${rejectingId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      <TopBar showBack title="Quest модерац" />
          ...
          <div className="space-y-3">
            {items.map((q) => (
              <div key={q.id} className="game-card p-4 space-y-3">
                ...
                <div>
                  <p className="text-sm font-bold text-foreground leading-snug">{q.title}</p>
                  <p className="text-xs text-muted-foreground mt-1 whitespace-pre-wrap">{q.description}</p>
                </div>

                <div className="flex items-center gap-2 text-xs flex-wrap">
                  <span className={`pill ${DIFF_COLOR[q.difficulty]}`}>{q.difficulty}</span>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-sm bg-card border border-border rounded-2xl p-5 space-y-3">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Жишээ нь: Тайлбар тодорхой бус"
            ...
            <div className="flex gap-2">
              <button onClick={() => setRejectingId(null)} className="btn-game-outline flex-1">
                Болих
              </button>
              <button
                onClick={reject}
